...
class TreeTopTarp:
    ...
    def reemplazar_si_es_visible(self, desde_este, desde_oeste, desde_norte, desde_sur):
        for i in range(self.largo):
            for j in range(self.largo):
                ...
                desde_o, desde_e, desde_n, desde_s = desde_oeste[i][j], desde_este[i][-j -
                                                                                      1], desde_norte[j][i], desde_sur[j][-i-1]
                punto = {desde_o, desde_e, desde_n, desde_s}
                if punto == {"_"}:
                    self.bosque_visible[i] = self.bosque_visible[i][:j] + \
                        "_" + self.bosque_visible[i][j+1:]
                elif len(punto - {"_"}) == 1:
                    nuevo = (punto - {"_"}).pop()
                    # try:
                    self.bosque_visible[i] = self.bosque_visible[i][:j] + \
                        nuevo + self.bosque_visible[i][j+1:]
                    ...
                    print("Fila: {fila}, columna: {columna}, valor preciso: {posta}".format(
                        fila=i, columna=j, posta=self.plano[i][j]))
                    print(punto)
    ...
    def panorama_pal_este(self, i, j, vidente=False):
        ...
        if vidente:
            self.vidente_variables(i, j, panorama, h_maxima, "Este")
        conteo = 0
        for arbol in panorama[i][j+1:]:
            if int(arbol) < h_maxima:
                conteo += 1
            else:
                conteo += 1
                break
    ...
    def panorama_pal_oeste(self, i, j, vidente=False):
        ...
        h_maxima = int(panorama[i][j])
        if vidente:
            self.vidente_variables(i, j, panorama, h_maxima, "Oeste")
        conteo = 0
    ...
    def panorama_pal_norte(self, j, i, vidente=False):
        ...
        h_maxima = int(panorama[i][j])
        if vidente:
            self.vidente_variables(i, j, panorama, h_maxima, "Norte")
        conteo = 0
        for arbol in panorama[i][j-1::-1]:
            if int(arbol) < h_maxima:
                conteo += 1
    ...
    def panorama_pal_sur(self, j, i, vidente=False):
        ...
        h_maxima = int(panorama[i][j])
        if vidente:
            self.vidente_variables(i, j, panorama, h_maxima, "Sur")
        conteo = 0
        for arbol in panorama[i][j+1:]:
            if int(arbol) < h_maxima:
                conteo += 1
    ...
    def vidente_variables(self, i, j, panorama, h_maxima, origen):
        print(" {}: {} en {} mirando para {}".format(
            origen, h_maxima, panorama[i][:], panorama[i][j+1:]))
...
print("-"*5)
assert respuestas_cortas.visibles_desde_derecha() == 11,  "Desde la Derecha se tendrían que ver 11 y se ven " + \
    str(respuestas_cortas.visibles_desde_derecha(True))
assert respuestas_cortas.visibles_desde_izquierda() == 11, "Desde la Izquierda se tendrían que ver 11 y se ven " + \
    str(respuestas_cortas.visibles_desde_izquierda(True))
assert respuestas_cortas.visibles_desde_arriba() == 10,   "Desde Arriba se tendrían que ver 10 y se ven " + \
    str(respuestas_cortas.visibles_desde_arriba(True))
assert respuestas_cortas.visibles_desde_abajo() == 8,     "Desde Abajo se tendrían que ver 8 y se ven " + \
    str(respuestas_cortas.visibles_desde_abajo(True))
...
assert respuestas_cortas.panorama_pal_este(
    1, 2) == 2,     "Este, si miro se tienen que ver 2 arboles y se ven: " + str(respuestas_cortas.panorama_pal_este(1, 2))
assert respuestas_cortas.panorama_pal_oeste(
    1, 2) == 1,    "Oeste, si miro se tiene que ver 1 arbol y se ven: " + str(respuestas_cortas.panorama_pal_oeste(1, 2))
assert respuestas_cortas.panorama_pal_norte(
    1, 2) == 1,    "Norte, si miro se tiene que ver 1 arbol y se ven: " + str(respuestas_cortas.panorama_pal_norte(1, 2))
assert respuestas_cortas.panorama_pal_sur(
    1, 2) == 2,      "Sur, si miro se tienen que ver 2 arboles y se ven: " + str(respuestas_cortas.panorama_pal_sur(1, 2))
assert respuestas_cortas.calculo_puntaje(
    1, 2) == 4,       "Puntaje debería ser 4"

print("Segunda Tanda, (3,2, True)")
vidente = False
assert respuestas_cortas.panorama_pal_este(
    3, 2, vidente) == 2,     "Este, si miro se tiene que ver 2 arboles y se ven: " + str(respuestas_cortas.panorama_pal_este(3, 2, vidente))
assert respuestas_cortas.panorama_pal_oeste(
    3, 2, vidente) == 2,    "Oeste, si miro se tiene que ver 2 arboles y se ven: " + str(respuestas_cortas.panorama_pal_oeste(3, 2, vidente))
assert respuestas_cortas.panorama_pal_norte(
    3, 2, vidente) == 2,    "Norte, si miro se tiene que ver 2 arboles y se ven: " + str(respuestas_cortas.panorama_pal_norte(3, 2, vidente))
assert respuestas_cortas.panorama_pal_sur(
    3, 2, vidente) == 1,      "Sur, si miro se tiene que ver 1 arbol y se ven: " + str(respuestas_cortas.panorama_pal_sur(3, 2, vidente))
assert respuestas_cortas.calculo_puntaje(
    3, 2) == 8,       "Puntaje debería ser 8"
assert respuestas_cortas.maximo_puntaje_del_interior(
) == 8,       "Puntaje debería ser 8"
...
RespuestaA, RespuestaB = respuestas_largas.contar_visibles(
), respuestas_largas.maximo_puntaje_del_interior()
