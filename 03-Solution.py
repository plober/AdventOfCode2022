...
class ItemsMochila:
    ...
    def suma_repetidos(inventarios):
        ...
        for lista in inventarios:
            total += ' abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ'.find(
                (set(lista[:len(lista)//2]) & set(lista[len(lista)//2:])).pop())
        return total
    ...
    def suma_tripletes(lista):
        ...
        for numero in range(0, len(lista), 3):
            repetido = set(lista[numero]) & set(
                lista[numero+1]) & set(lista[numero+2])
            suma_total += ' abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ'.find(
                repetido.pop())
            # print(suma_total)
        return suma_total
