...
class Inputs:
    def parseo_situacion(situacion):
        ...
        for caracter_columna in range(len(situacion[0]))[1::4]:
            traspuesto = ['{: <{}}'.format(
                fila, largo)[caracter_columna] for fila in situacion]
            diccionario_pilas[int(traspuesto[0])] = ''.join(
                traspuesto[1:]).strip()
...
class Respuesta:
    def ejecucion_movimientos(diccionario_pilas, instrucciones):
        ...
        for comando in instrucciones:
            ...
            for _ in range(cuantos):
                diccionario_pilas[hacia] = diccionario_pilas[hacia] + \
                    diccionario_pilas[desde][-1]
    ...
    def ejecucion_movimientos_b(diccionario_pilas, instrucciones):
        ...
        for comando in instrucciones:
            [cuantos, desde, hacia] = comando
            diccionario_pilas[hacia] = diccionario_pilas[hacia] + \
                diccionario_pilas[desde][-cuantos:]
...
respuesta_corta_a = Respuesta.ejecucion_movimientos(Inputs.parseo_situacion(
    corto_inicial.copy()), Inputs.parseo_movimientos(corto_movimientos.copy()))
respuesta_a = Respuesta.ejecucion_movimientos(Inputs.parseo_situacion(
    largo_inicial.copy()), Inputs.parseo_movimientos(largo_movimientos.copy()))
respuesta_corta_b = Respuesta.ejecucion_movimientos_b(Inputs.parseo_situacion(
    corto_inicial.copy()), Inputs.parseo_movimientos(corto_movimientos.copy()))
respuesta_b = Respuesta.ejecucion_movimientos_b(Inputs.parseo_situacion(
    largo_inicial.copy()), Inputs.parseo_movimientos(largo_movimientos.copy()))

assert Inputs.parseo_situacion(corto_inicial.copy()) == {
    1: "ZN", 2: "MCD", 3: "P"}
assert Inputs.parseo_movimientos(corto_movimientos.copy()) == \
    [[1, 2, 1], [3, 1, 3], [2, 2, 1], [1, 1, 2]]

assert Respuesta.ejecucion_movimientos(
    {1: "ZN", 2: "MCD", 3: "P"},
    [[1, 2, 1], [3, 1, 3], [2, 2, 1], [1, 1, 2]]) == "CMZ"

print("	La respuesta del Ejemplo A es " + str(respuesta_corta_a))
print("	La respuesta A es " + str(respuesta_a))

assert Respuesta.ejecucion_movimientos_b(Inputs.parseo_situacion(
    corto_inicial.copy()), Inputs.parseo_movimientos(corto_movimientos.copy())) == "MCD"
